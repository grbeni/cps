...
/**
* A sequence of UvegHaz instances.
*/
public final class UvegHazSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public UvegHazSeq (int initialMaximum) {
        super(UvegHaz.class, initialMaximum);
    }

    public UvegHazSeq (Collection elements) {
        super(UvegHaz.class, elements);
    }

    public  UvegHaz get(int index) { 
        return (UvegHaz) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, UvegHaz.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' UvegHaz objects (beyond the original size of this object)
